...
class VGGLossX(nn.Module):
    def __init__(self, normalize=True, mask=False, relative=False, style=False):
        ...
        self.vgg = VGG16FeatureExtractor().to('mps')
        self.criterion = nn.L1Loss().to('mps') if not relative else l1_relative
        self.use_style = style
        self.use_mask= mask
        self.relative = relative

        if normalize:
            self.normalize = MeanShift([0.485, 0.456, 0.406], [0.229, 0.224, 0.225], norm=True).to('mps')
        else:
            self.normalize = None
