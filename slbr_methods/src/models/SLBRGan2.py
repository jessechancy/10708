...
from src.networks.discriminator import patchgan, sngan, maskedsngan
...
class SLBR(BasicModel):
    def __init__(self,**kwargs):
        ...
        self.discriminator = maskedsngan()
        ...
        learnable_params = 0
        for x in filter(lambda p: p.requires_grad, self.discriminator.parameters()):
            learnable_params += torch.sum(x.size())
        print("LEARNABLE DISCRIMINATOR PARAMS", learnable_params)

        self.discriminator.to(self.device)
        self.BCE_loss = nn.BCELoss().to(self.device)
        print('==> Total params discriminator: %.2fM' % (sum(p.numel() for p in self.discriminator.parameters())/1000000.0))
    ...
    def train(self,epoch):
        ...
        for i, batches in enumerate(self.train_loader):
            ...
            if ((i+1)%3) == 0:
                # discriminator is trying to tell the difference between the output of the model and the ground truth
                self.discriminator_optimizer.zero_grad()

                D_real = self.discriminator(self.norm(inputs), self.norm(target), mask)
                D_real_loss = self.BCE_loss(D_real, torch.ones_like(D_real))

                outputs = self.model(self.norm(inputs))
                refined_output = outputs[0][0]
                D_fake = self.discriminator(self.norm(inputs), refined_output, mask)
            ...
            D_fake = self.discriminator(self.norm(inputs), outputs[0][0], mask)
            G_loss = self.BCE_loss(D_fake, torch.ones_like(D_fake))
            
            D_percent = 0.3
            print(G_loss, self.args.lambda_l1*(coarse_loss+refine_loss) + self.args.lambda_mask * (mask_loss)  + style_loss)
            total_loss = (1-D_percent)*(self.args.lambda_l1*(coarse_loss+refine_loss) + self.args.lambda_mask * (mask_loss)  + style_loss) + D_percent*G_loss
            ...
            if i % 100 == 0:
                ...
                self.record('train/disc_loss', D_loss, current_index)
                self.record('train/g_loss', G_loss, current_index)
