...
class generator(nn.Module):
    ...
    def __init__(self, n_channels, n_classes, bilinear=True):
        ...
        self.post_process_x1=nn.Sequential(
          nn.Conv2d(64+6, 64, 3, 1, 1),
          nn.BatchNorm2d(64),
          nn.LeakyReLU(0.2),
          nn.Conv2d(64, 128, 3, 1, 1),
          )
        self.post_process_x2=nn.Sequential(
          nn.BatchNorm2d(128),
          nn.LeakyReLU(0.2),
          nn.Conv2d(128, 128, 3, 1, 1),
          )
        self.post_process_x3=nn.Sequential(
          nn.BatchNorm2d(128),
          nn.LeakyReLU(0.2),
          nn.Conv2d(128, 128, 3, 1, 1),
          )
        self.post_process_x4=nn.Sequential(
          nn.BatchNorm2d(128),
          nn.LeakyReLU(0.2),
          nn.Conv2d(128, 128, 3, 1, 1),
          )
        self.post_process_x5=nn.Sequential(
          nn.BatchNorm2d(128),
          nn.LeakyReLU(0.2),
    ...
    def forward(self, x0):
        ...
        xy1 = self.post_process_y1(xx)
        xy2 = self.post_process_y2(xy1)
        ca = self.contextual_attention(xy2, xy2)
        xy3 = self.post_process_y3(ca)

        # gated conv
        xx1=self.post_process_x1(xx)
        xx2=self.post_process_x2(xx1)
        xx3=self.post_process_x3(xx1+xx2)
        xx4=self.post_process_x4(xx2+xx3)
...
class CAWDNet(object):
    def __init__(self, args):
        ...
        print(sum(p.numel() for p in self.G.parameters()))
