...
class generator(nn.Module):
    ...
    def __init__(self, n_channels, n_classes, bilinear=True):
        ...
        self.blur = torchvision.transforms.GaussianBlur(kernel_size=(3,3), sigma=(1,3))

        self.post_process_1=GatedConvolution(chan+6, chan*2, kernel_size=3, padding=1, activation="leaky_relu")
        
        self.post_process_2=GatedConvolution(chan*2, chan*2, kernel_size=3, padding=1, activation="leaky_relu")

        self.post_process_3=GatedConvolution(chan*2, chan*2, kernel_size=3, padding=1, activation="leaky_relu")

        self.post_process_4=GatedConvolution(chan*2, chan*2, kernel_size=3, padding=1, activation="leaky_relu")
    ...
    def forward(self, x0):
        ...
        # mask = self.blur(mask)*1.1
        mask=self.sg(mask)
        a=self.sg(a)
        w=self.sg(w)
        a=mask*a
        I_watermark=(x0-a*w)/(1.0-a+1e-8)
