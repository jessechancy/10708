...
class generator(nn.Module):
    ...
    def forward(self, x0):
        ...
        # z = torch.normal(0, 1, size=I_watermark.size()).to(device)
        # noise = torch.mean(var) * z * 0.0001
        # I_watermark_noised = I_watermark + noise*mask
        ...
        return I,mask,a,w,I_watermark,None
        ...
        for epoch in range(self.epoch):
            ...
            for iter, (x_, y_, mask, balance, alpha, w) in enumerate(self.data_loader):
                ...
                if ((iter+1)%3)==0 :
                  ...
                  G_ ,g_mask, g_alpha, g_w,I_watermark,mean_var= self.G(x_)
                  D_fake = self.D(x_,G_)
                ...
                if((iter+1)%100) ==0:
                    ...
                    writer.add_scalar('variance', mean_var, iter_all)

                if ((iter + 1) % 100) == 0:
                    print("Epoch: [%2d] [%4d/%4d] D_loss: %.8f, G_loss: %.8f" %
                          ((epoch + 1), (iter + 1), self.data_loader.dataset.__len__() // self.batch_size, D_loss.item(), G_writer))
                    print(w_loss, alpha_loss, mask_loss, I_watermark_loss, I_watermark2_loss, vgg_loss, mean_var)
