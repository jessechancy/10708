...
G=generator(3,3)
G.eval()
G.load_state_dict(torch.load(os.path.join('WDNet_G.pkl'), map_location=torch.device('mps')))
...
for img_id in ids:
  ...
  img_source=torch.unsqueeze(img_source.to(torch.device('mps')),0)
  st=time.time()
  pred_target,mask,alpha,w,I_watermark,_=G(img_source)
