...
class generator(nn.Module):
    ...
    def __init__(self, n_channels, n_classes, bilinear=True):
        ...
        chan = 48
        self.inc = GatedDoubleConv(n_channels, chan)
        self.down1 = GatedDown(chan, chan*2)
        self.down2 = GatedDown(chan*2, chan*4)
        self.down3 = GatedDown(chan*4, chan*8)
        self.down4 = GatedDown(chan*8, chan*8)
        self.up1 = GatedUp(chan*16, chan*4, bilinear)
        self.up2 = GatedUp(chan*8, chan*2, bilinear)
        self.up3 = GatedUp(chan*4, chan, bilinear)
        self.up4 = GatedUp(chan*2, chan, bilinear)
        self.dilation=nn.Sequential(
          GatedConvolution(chan*8, chan*8, padding=2, dilation=2, activation="leaky_relu"),
          GatedConvolution(chan*8, chan*8, padding=4, dilation=4, activation="leaky_relu"),
          GatedConvolution(chan*8, chan*8, padding=6, dilation=6, activation="leaky_relu")
        )
        self.outw = OutConv(chan, 3)
        self.outa = OutConv(chan, 1)
        self.out_mask = OutConv(chan, 1)
        self.sg=nn.Sigmoid()
        self.other=OutConv(chan, chan)


        self.post_process_x = nn.Sequential(
            GatedConvolution(chan+6, chan*2, kernel_size=3, padding=1, activation="leaky_relu"),
            GatedConvolution(chan*2, chan*2, kernel_size=3, padding=1, activation="leaky_relu"),
            GatedConvolution(chan*2, chan*4, kernel_size=3, padding=1, activation="leaky_relu"),
            GatedConvolution(chan*4, chan*4, kernel_size=3, padding=1, activation="leaky_relu"),
        )
        
        self.post_process_ca1 = nn.Sequential(
            GatedConvolution(chan+6, chan*2, kernel_size=3, padding=1, activation="leaky_relu"),
            GatedConvolution(chan*2, chan*2, kernel_size=3, padding=1, activation="leaky_relu"),
            GatedConvolution(chan*2, chan*4, kernel_size=3, padding=1, activation="leaky_relu"),
        )
        ...
        self.post_process_ca2 = nn.Sequential(
            GatedConvolution(chan*4, chan*4, kernel_size=3, padding=1, activation="leaky_relu")
        )

        self.combine=nn.Sequential(
            GatedConvolution(chan*8, chan*4, kernel_size=3, padding=1, activation="leaky_relu"),
            GatedConvolution(chan*4, chan*2, kernel_size=3, padding=1, activation="leaky_relu"),
            GatedConvolution(chan*2, 3, kernel_size=3, padding=1, activation="leaky_relu"),
            nn.Sigmoid(),
        )

        # self.post_process_5=nn.Sequential(
        #   nn.Conv2d(chan*2, 3, kernel_size=3, stride=1, padding=1),
        #   nn.Sigmoid(),
        # )

        # self.post_process_1=nn.Sequential(
        #   nn.Conv2d(chan+6, chan, 3, 1, 1),
        #   nn.BatchNorm2d(chan),
        #   nn.LeakyReLU(0.2),
    ...
    def forward(self, x0):
        ...
        xx = torch.cat([other,I_watermark,x0],1)
        xx1 = self.post_process_x(xx)

        pre_ca = self.post_process_ca1(xx)
        mask_s = nn.functional.interpolate(mask, (pre_ca.shape[2], pre_ca.shape[3]))
        post_ca = self.contextual_attention(pre_ca, pre_ca, mask_s)
        xx2 = self.post_process_ca2(post_ca)

        I_watermark2 = self.combine(torch.cat([xx1,xx2], axis=1))
...
class CAGatedWDNet(object):
    def __init__(self, args):
        ...
        if self.gpu_mode:
            self.sample_z_, self.sample_y_ = self.sample_z_.to(device), self.sample_y_.to(device)
    ...
    def train(self):
        ...
        for epoch in range(self.epoch):
            ...
            for iter, (x_, y_, mask, balance, alpha, w) in enumerate(self.data_loader):
                ...
                if self.gpu_mode:
                    x_, y_, mask, balance, alpha, w=x_.to(device), y_.to(device), mask.to(device), balance.to(device), alpha.to(device), w.to(device)
                ...
                if ((iter + 1) % 100) == 0:
                    ...
                    print(w_loss, alpha_loss, mask_loss, I_watermark_loss, I_watermark2_loss, vgg_loss)
            self.save()
